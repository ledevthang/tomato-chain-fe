...
import CustomCardProduct from '~/components/common/CustomCardProduct.vue'
export default {
    ...
    const companyInfo = dataConverted.filter(company => company.companyID === id)[0]
  ...
  methods: {
    ...
    async getProduct () {
      ...
      this.companyProducts = dataConverted.filter(product => product.Provider === this.companyInfo.currentAddress)
    },
    reset () {
